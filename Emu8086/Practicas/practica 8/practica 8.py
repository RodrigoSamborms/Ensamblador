# Definir el conjunto de instrucciones del microprocesador 8086
INSTRUCCIONES_8086 = [
...
def calcular_tamano_instruccion(instruccion):
    ...
    # Asumimos que cada instrucción tiene al menos un byte
    tamano = 1
    if ' ' in instruccion:
        # Si la instrucción tiene operandos, sumamos un byte por cada operando
...
def calcular_direccion_memoria(direccion_base, lineas_asm):
    ...
    for linea in lineas_asm:
        instruccion = linea.strip().split(' ')[0]  # Obtener la instrucción de la línea
        tamano = calcular_tamano_instruccion(instruccion)
        direcciones.append(f"{direccion_actual:04X}")  # Agregar la dirección actual al resultado
        direccion_actual += tamano  # Incrementar la dirección por el tamaño de la instrucción
    return direcciones
...
def main():
    ...
    # Solicitar al usuario la ruta del archivo .asm
    ...
    try:
        ...
        # Escribir las direcciones de memoria en el archivo .lst
